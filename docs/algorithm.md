...
| 17 | palette recomputed each iteration | Correct-by-construction rather than incrementally maintained. Construction runs once per restart while the tabu phase runs until the deadline, so the construction's `O(n K deg)` per round is not what the wall clock is spent on — and an incrementally maintained palette would have to be invalidated correctly on every eviction and rollback in `_seat`, which is exactly the kind of bookkeeping that produced the `can_swap` and `_perturb` bugs elsewhere in this codebase. |
...
| 25 | chain depth 3 | **Not tuned — measured and found not to matter.** Sweeping depth over {1,2,3,4} on 55 generator instances that survive the certificates and still corner a task solved 9/55 at *every* depth, with no meaningful runtime difference. 3 is a conservative bound on the recursion; on generator-shaped instances this phase is insurance, not a workhorse. |
